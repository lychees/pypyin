...
def peakProb(input, dist, minTau0 = 0, maxTau0 = 0):
    lin = len(input)
    minTau = 2
    maxTau = lin
    ...
    if(minTau0 > 0 and minTau0 < maxTau0):
        minIdx = minIdx0
    if(maxTau0 > 0 and maxTau0 < lin and maxTau0 > minTau):
        maxTau = maxTau0
    ...
    distribution = dist[:nThreshold]
    thresholds = np.arange(0, nThreshold, dtype = np.float64) * 0.01 + 0.01
    
    currThresholdIdx = nThreshold - 1
    tau = minTau
    ...
    peakProb = np.zeros((lin), dtype = np.float64)
    
    while(tau + 1 < maxTau):
        if(input[tau] < thresholds[-1] and input[tau + 1] < input[tau]):
            while(tau + 1 < maxTau and input[tau + 1] < input[tau]):
                tau += 1
            # now tau is local minimum
            if(input[tau] < minVal and tau > 2):
                minVal = input[tau]
                minIdx = tau
            currThresholdIdx = nThreshold - 1
            while(currThresholdIdx > -1 and thresholds[currThresholdIdx] > input[tau]):
                peakProb[tau] += distribution[currThresholdIdx]
                currThresholdIdx -= 1
    ...
    if(peakProb[minIdx] > 1.0):
        print("WARNING: Bad prob value. Returning all zeros.", f = sys.stderr)
        return(np.zeros((lin), dtype = np.float64))
    nonPeakProb = 1.0
    if(sumProb > 0):
        for i in range(minTau, maxTau):
            peakProb[i] = peakProb[i] / sumProb * peakProb[minIdx];
            nonPeakProb -= peakProb[i]
        peakProb[minIdx] += nonPeakProb * minWeight
    
    for idx, val in enumerate(peakProb):
        if(val > 0.99):
            peakProb[idx] = 0.99
